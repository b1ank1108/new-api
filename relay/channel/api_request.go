...
// copyAllRequestHeaders copies all incoming headers except unsafe system/auth headers.
func copyAllRequestHeaders(c *gin.Context, targetHeader *http.Header) {
	if c == nil || c.Request == nil || targetHeader == nil {
		return
	}
	for headerKey, headerValues := range c.Request.Header {
		if shouldSkipPassthroughHeader(headerKey) {
			continue
		}
